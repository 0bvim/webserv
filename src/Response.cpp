...
Response::Response(Request &request, const ServerConfig &config)
...
void Response::_fillStatusMap() {

  this->statusMap[HttpStatus::ZERO] = "UNKNOW STATUS";
  this->statusMap[HttpStatus::CONTINUE] = "100 Continue";
  this->statusMap[HttpStatus::OK] = "200 OK";
  this->statusMap[HttpStatus::CREATED] = "201 Created";
  this->statusMap[HttpStatus::NO_CONTENT] = "204 No Content";
  this->statusMap[HttpStatus::MOVED_PERMANENTLY] = "301 Moved Permanently";
  this->statusMap[HttpStatus::BAD_REQUEST] = "400 Bad Request";
  this->statusMap[HttpStatus::FORBIDDEN] = "403 Forbidden";
  this->statusMap[HttpStatus::NOT_FOUND] = "404 Not Found";
  this->statusMap[HttpStatus::NOT_ALLOWED] = "405 Method Not Allowed";
  this->statusMap[HttpStatus::TIMEOUT] = "408 Request Timeout";
  this->statusMap[HttpStatus::CONFLICT] = "409 Conflict";
  this->statusMap[HttpStatus::PAYLOAD_TOO_LARGE] = "413 Payload Too Large";
  this->statusMap[HttpStatus::SERVER_ERR] = "500 Internal Server Error";
  this->statusMap[HttpStatus::NOT_IMPLEMENTED] = "501 Not Implemented";
  this->statusMap[HttpStatus::SERVICE_UNAVAILABLE] = "503 Service Unavailable";
}
...
void Response::_generateStatusLine() {
  ...
  this->_response.statusLine += this->statusMap[this->_status];
}
  ...
  for (size_t j = 0; j < server.locations.size(); j++) {
    if (request.uri.find(server.locations[j].path) != std::string::npos) {
      for (size_t k = 0; k < server.locations[j].cgi.size(); k++) {
        if (request.uri.find(server.locations[j].cgi[k].extension) !=
            std::string::npos) {
